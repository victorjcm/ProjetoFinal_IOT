...
def on_message(client, userdata, msg):
    ...
    if topic == FEED_BOTAO_VIRTUAL:
        ...
        # Envia o comando absoluto correspondente
        cmd = "ABRIR" if cortina_aberta else "FECHAR"
        asyncio.run_coroutine_threadsafe(enviar_comando_coap(cmd), loop)
...
class SensorResource(resource.Resource):
    async def render_post(self, request):
        ...
        cursor.execute(
            "INSERT INTO leituras_luz (data_hora, valor) VALUES (?, ?)",
            (agora, valor_luz),
        )
        conn.commit()
        ...
        if modo_atual == "AUTOMATICO":
            if valor_luz > limite_luz and cortina_aberta:
                ...
                asyncio.create_task(enviar_comando_coap("FECHAR"))
                ...
                asyncio.create_task(enviar_comando_coap("ABRIR"))
...
async def main():
    ...
    protocolo_coap = await aiocoap.Context.create_server_context(
